...
def determine_rating_of_combo(combo, positions_in_order, rating_dict, number_in_combo) :
    total = 0
    for i in range(number_in_combo) :
        total += rating_dict[combo[i]][positions_in_order[i]]
    return total

def get_correct_general_placements(p_order, p_possibility) :
    points = 0
    for i, p_po in enumerate(p_possibility) :
        r_po, c_po = current_data[team+'_Players'][p_po]['Position'], p_order[i]
        if not ((r_po[0] in ['R', 'L'] and c_po[0] == 'C') or (r_po[0] == 'C' and c_po[0] in ['R', 'L'])) :
            points += 1
    return points
